...
func _runGravityPhysics(delta: float) -> void:
	if (!_character_body.is_on_floor()):
		...
		if (_gravitational_authority != null && _gravitational_authority.has_method("getGravity")):
			_character_body.velocity.y -= _gravitational_authority.getGravity() * delta
		else:
...
func _ready() -> void:
	...
	if (!SIG_jump.get_connections().size() > 0):
	...
	# Ensure the correct collision collision layer is set
	_character_body.set_collision_layer(2)
...
func getDefaultGravitationalAuthorityNode() -> Node:
	# The node structure for a WorldData instance (Which is where character
	...
	#
	# WorldData is the desired node that contains the script with 'getGravity()'
